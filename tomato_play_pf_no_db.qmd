...
I will use the `tidyverse` package for analysis. I will use `duckdb` here because it makes queries very fast with very little overhead.[^1]
...
```{r}
#| message: false
movies <- read_csv('archive/rotten_tomatoes_movies.csv')
reviews <- read_csv('archive/rotten_tomatoes_movie_reviews.csv')
```
...
The result above tells us that are duplicate rows in the `movies` table. If we focus on those with a value in `title`, it seems there are complete duplicates (i.e., all rows are duplicated). So we can make a new table with unique entries.
...
There are also some duplicates in `reviews`. Let's make `reviews_unique`:
...
What values does `reviewState` take? Are there missing values?
